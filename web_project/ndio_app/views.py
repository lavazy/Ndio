    ...
    if network_providers_list is not None:
        for items in network_providers_list:
            ...
            response = requests.get(
                f"https://apitest.axxess.co.za/calls/rsapi/getNetworkProviderProducts.json?strSessionId={session_id}&guidNetworkProviderId={items}",
...
def home(request):
    ...
    if request.method == "POST":
        ...
        print(f"Checking fibre availability for address: {address}")

        if network_providers_list is not None:
            fibre_is_available = True
            network_provider_products_list = get_network_provider_products(
                address=address
            )

            # Attach price to each product from the DB
            for product in network_provider_products_list:
                product_id = product.get("guidProductId")
                try:
                    fibre_product = FibreProduct.objects.get(product_id=product_id)
                    product["price"] = fibre_product.product_price
                    product["product_speed"] = fibre_product.product_speed
                except FibreProduct.DoesNotExist:
                    product["price"], product["product_speed"] = None  # Or "N/A"
...
def order_details(request):
    """Handles the order details page and processes form submission."""
    ...
    if request.method == "POST":
        ...
        if form.is_valid() and form_order.is_valid():
            try:
                # Try to get the existing UserDetail for this user
                existing_detail = request.user.user_details
                form = forms.UserDetailForm(request.POST, instance=existing_detail)
                client = form.save(commit=False)
            except UserDetail.DoesNotExist:
                # Check if phone number or ID number already exists for another user
                if (
                    ...
                    form.errors["phone_number"] = [
                        "User detail with this Phone number already exists."
                    ]
                    return render(
                        request,
                        "ndio_app/order_details.html",
                        {"form": form, "form_order": form_order},
                    )

                if (
                    UserDetail.objects.filter(id_number=form.cleaned_data["id_number"])
                    .exclude(user=request.user)
                    .exists()
                ):
                    form.errors["id_number"] = [
                        "User detail with this Id number already exists."
                    ]
                    return render(
                        request,
                        "ndio_app/order_details.html",
            ...
            # Get product and network provider info
            product = FibreProduct.objects.filter(product_id=fibre_product_id).first()
            if not product:
                messages.error(request, "Fibre product not found.")
                return redirect("order_details")

            network = NetworkProvider.objects.filter(
                name=product.network_provider
            ).first()
            if not network:
                messages.error(request, "Network provider not found.")
                return redirect("order_details")
            ...
            logger.error("User Detail Form Errors: %s", form.errors)
            logger.error("Order Form Errors: %s", form_order.errors)
            messages.error(request, "Please correct the errors in the form.")

    else:
        # Handle GET: prefill address fields
        ...
        try:
            ...
            form = forms.UserDetailForm(initial={"referred_by": referrer})

        form_order = forms.OrderForm(initial=order_initial_data)

    return render(
        request, "ndio_app/order_details.html", {"form": form, "form_order": form_order}
    )
...
def payment_view(request):
    ...
    product_final_price = product_price.quantize(Decimal("0.00"))
    wifi_router_price = Decimal(350)
    final_wifi_router_price = wifi_router_price.quantize(Decimal("0.00"))
    total_price = Decimal(product_final_price + wifi_router_price)
    final_total_price = total_price.quantize(Decimal("0.00"))

    # Yoco Public Key
    public_key = settings.YOCO_PUBLIC_KEY

    # Create an unbound form with initial data
    form = PaymentForm(initial={"amount": final_total_price})

    context = {
        "yoco_public_key": public_key,
        "currency": "ZAR",
        "router_price": final_wifi_router_price,
...
def user_account(request):
    ...
    product_price = Decimal(product.product_price)
    final_product_price = product_price.quantize(Decimal("0.00"))
    print(f"Price: {final_product_price}")
    ...
    context = {"ref_code": ref_code, "product_price": final_product_price}
...
def monthly_payments(request):
    fibre_product_id = request.session.get("fibre_product")
    print(f"Product ID: {fibre_product_id}")
    ...
    # Create an unbound form with initial data
    form = PaymentForm(initial={"amount": product_final_price})
    ...
    return render(request, "accounts/monthly_payments.html", context=context)
